...
<script type="text/javascript">
	function conf() {
		var pil = confirm("Apakah Anda ingin mengonfirmasi bahwa data Anda sudah valid ? Setelah dikonfirmasi data tidak bisa diubah lagi");
		if (pil == true) {
			alert("Konfirmasi berhasil! Data menunggu diverifikasi oleh Panitia");
		} else {
...
<div class="verif">
	<div class="col-md-6 konf_bio">
		...
		<br><p></p><br>
		<table class="table">
		<?php while ($x = mysqli_fetch_assoc($exec_bio)) { ?>
			<tr>
				<th>Nama</th>
				<td>:</td>
				<td><?= $x['name']; ?></td>
		...
		<a href="edit.bio.<?= $x['id']; ?>" class="btn btn-warning">Edit Data</a>
			<?php } ?>
	</div>
	...
	<div class="col-md-12 konf_nilai">
		...
		<table class="table">
			...
			<tr class="nilai">
				...
				<?php while ($n = mysqli_fetch_assoc($exec_nilai)) { ?>
				<th><?= $n['bindo']; ?></th>
				<th><?= $n['ipa']; ?></th>
				<th><?= $n['mtk']; ?></th>
				<th><?= $n['binggris']; ?></th>
			...
			<tr>
				<th colspan="5"><h4>2. Data Kejuruan</h4></th>
			</tr>
			<tr>
		...
		<a href="edit.nilai.<?= $n['id']; ?>" class="btn btn-warning">Edit Data</a>
		<!--button type="button" class="btn btn-info" style="float:right;" onclick="conf()">Konfirmasi Data Saya</button-->
		<a href="konfirmasi.data.<?= $n['id_user']; ?>" class='btn btn-info' style="float:right;">Konfrimasi Data Saya</a>
		<?php } ?>
		<p style="color:red;padding:15px;margin-top:10px;background:#fff;"><strong style="color:#000;">NOTE : </strong>Jika Data Anda sudah benar dan valid, klik tombol <strong>Konfirmasi Data Saya</strong></p>
	</div>
</div>